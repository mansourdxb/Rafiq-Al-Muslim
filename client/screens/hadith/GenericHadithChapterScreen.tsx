...
export default function GenericHadithChapterScreen({ bookKey }: Props) {
  ...
  const { chapters } = useHadithBook(bookKey);
  ...
  const chapter = chapters?.[chapterId] as ChapterData | undefined;
  ...
  const handleContentSizeChange = useCallback(() => {
    if (needsScroll && !hasScrolled.current && flatListRef.current) {
      ...
      setTimeout(() => {
        flatListRef.current?.scrollToIndex({ index: highlightIndex, animated: false, viewPosition: 0 });
      }, 50);
    }
  ...
  const onToggleFavorite = async (favId: string, payload: { bookTitle: string; chapterTitle: string; arabicText: string; idInBook: number | string | undefined }) => {
    ...
    if (next[favId]) {
      ...
      next[favId] = true;
      nextItems[favId] = { bookId: bookKey, chapterId, idInBook: payload.idInBook, bookTitle: payload.bookTitle, chapterTitle: payload.chapterTitle, arabicText: payload.arabicText, savedAt: Date.now() };
      setFavorites(next); setFavoriteItems(nextItems);
      await saveFavorites(next); await saveFavoriteItems(nextItems);
      showToast("\u062a\u0645 \u0627\u0644\u062d\u0641\u0638");
    }
  ...
  const showShareMenu = useCallback((hadithId: number | string, cleaned: string, shareText: string) => {
    ...
    const handleAction = (index: number) => {
      if (index === 0) {
        ...
        setTimeout(() => shareHadithImage(hadithImageRef), 400);
      } else if (index === 2) {
        setImageData({ hadithText: cleaned, hadithNumber: hadithId, source: metadataTitle, author: metadataAuthor, chapter: chapterTitle });
        setTimeout(() => saveHadithImage(hadithImageRef), 400);
      }
    };
    ...
    <View style={styles.root}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          const favId = getFavoriteId(bookKey, chapterId, hadithId);
